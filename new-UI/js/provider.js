...
async function onLoad() {
    try {
        showLoadingOverlay();
        p_id = getIdValue();
        await fetchAndDisplayProviderData();
        await fetchAndDisplayPatientData();
        apiUrl5 = `http://localhost:3000/image/${p_id}`;
        await fetchAndLoadImage(apiUrl5);
        contractUrl = `select distinct(ObjectId) from InformedConsent where Subject = "${p_id}"`;
...
async function fetchAndDisplayConsentData(patientId) {
    ...
    for (const element of contracts) {
        try {
            ...
            const consents = await contract.getConsentsBySubject(p_id);
