...
document.getElementById("login-button").addEventListener("click", function () {
    ...
    if (document.getElementById("patient-id").value.startsWith('ICA') && document.getElementById("patient-id").value == document.getElementById("password").value) {
        ica_id = pid;
        document.getElementById("login-form").style.display = "none";
        fetchAndDisplayInsuranceAgentDetailsData();
        fetchAndDisplayPatientData();
        fetchAndDisplayConsentData();
        apiUrl5 = `http://localhost:3000/image/${ica_id}`;
...
async function fetchAndDisplayInsuranceAgentDetailsData() {
    try {
        query = `select * from insuranceAgent where ID = '${ica_id}'`;
        patientData = await executeQuery(query);
        patientID = patientData.data[0].patient_id;
        const patientDetailsContainer = document.getElementById('insurance-details-container');
...
async function fetchAndDisplayPatientData() {
    try {
        ...
        const ecQuery = `select distinct Patient from final_consents where Subject = '${ica_id}'`;
...
//     try {
//         const query2 = `select * from final_consents where Subject = '${ica_id}'`;
//         const patientData = await executeQuery(query2);
